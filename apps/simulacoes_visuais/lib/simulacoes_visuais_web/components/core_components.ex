defmodule SimulacoesVisuaisWeb.CoreComponents do
  @moduledoc """
  ...
  @doc """
  Renders the FBE detail sidebar and/or 3D iframe block (shared between 2D and 3D views).

  Use `part: :sidebar` inside the flex layout; use `part: :iframe` after the flex for the iframe block.
  """
  attr :part, :atom,
    required: true,
    values: [:sidebar, :iframe],
    doc: "Which part to render (:sidebar or :iframe)"
  ...
  def fbe_detail_panel(assigns) do
    ~H"""
    <%= if @part == :sidebar do %>
      <div class="lg:w-80 border-t lg:border-t-0 lg:border-l border-base-200 flex flex-col bg-base-200/30">
        ...
        <div class="flex-1 overflow-y-auto p-4">
          <%= if @fbe_descricao_long && @fbe_descricao_long != "" do %>
    ...
    <% end %>
    <%= if @part == :iframe && @static_3d_url do %>
      <div
        id={@id}
        phx-hook="FbeDetailIframe"
        data-fbe-id={@selected_fbe}
        data-fbe-facts={Jason.encode!(@iframe_facts)}
        class="border-t border-base-200 p-4"
      >
        <p class="text-xs text-base-content/60 mb-2">
          Modelo 3D detalhado: {@fbe_label}
        </p>
        <iframe
          src={@static_3d_url}
          class="w-full h-[480px] rounded border border-base-300 bg-base-200"
          title={"Modelo 3D: #{@fbe_label}"}
        >
        </iframe>
      </div>
    <% end %>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SimulacoesVisuaisWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SimulacoesVisuaisWeb.Gettext, "errors", msg, opts)
    end
  end
